...
impl Radar {
    /// Create a Radar sensor from a carla-cxx SensorWrapper.
    ...
    /// Create a Radar sensor from an actor by casting.
    ...
    pub fn listen_async(&self) -> Result<tokio::sync::mpsc::Receiver<RadarData>, SensorError> {
        ...
        todo!("Radar::listen_async not yet implemented with carla-cxx FFI")
    }
...
impl ActorT for Radar {
    fn id(&self) -> ActorId {
        // Radar doesn't have a direct GetId method, need to cast to Actor
        let actor_ptr = carla_cxx::ffi::Sensor_CastToActor(self.inner.get_inner_sensor());
        if actor_ptr.is_null() {
            panic!("Internal error: Failed to cast Radar to Actor");
        }
    ...
    fn bounding_box(&self) -> crate::geom::BoundingBox {
        // Radar doesn't have a direct GetBoundingBox method, need to cast to Actor
        let actor_ptr = carla_cxx::ffi::Sensor_CastToActor(self.inner.get_inner_sensor());
        if actor_ptr.is_null() {
            panic!("Internal error: Failed to cast Radar to Actor");
        }
...
impl SensorT for Radar {
    ...
    fn attribute(&self, name: &str) -> Option<String> {
        ...
        todo!("Radar::get_attribute not yet implemented - missing FFI function Sensor_GetAttribute")
    }
