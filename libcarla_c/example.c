...
void spawn_vehicle_example(carla_world_t* world) {
    ...
    carla_actor_blueprint_t* vehicle_bp = carla_blueprint_library_find(bp_lib, "vehicle.tesla.model3");
    if (!vehicle_bp) {
        printf("Tesla Model 3 not found, trying another vehicle...\n");
        // Try to find any vehicle
        size_t filter_count = 0;
        carla_actor_blueprint_t** vehicles = carla_blueprint_library_filter(bp_lib, "vehicle.*", &filter_count);
        if (vehicles && filter_count > 0) {
            ...
            for (size_t i = 1; i < filter_count; i++) {
                // Note: Don't free individual blueprints as they're managed by the library
            }
            free(vehicles);
        }
    }
    ...
    if (spawn_points && carla_transform_list_size(spawn_points) > 0) {
        carla_transform_t spawn_transform = carla_transform_list_get(spawn_points, 0);
        
        // Try to spawn the vehicle
        carla_spawn_result_t result = carla_world_try_spawn_actor(world, vehicle_bp, &spawn_transform, NULL);
