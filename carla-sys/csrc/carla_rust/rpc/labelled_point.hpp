    ...
    namespace rpc {
        ...
        public:
            ...
            FfiLabelledPoint(LabelledPoint &&orig)
                :
                location(std::move(orig._location)),
                label(std::move(orig._label))
            {}
        };

        static_assert(sizeof(FfiLabelledPoint) == sizeof(LabelledPoint), "FfiLabelledPoint size is incorrect");
    }
}
