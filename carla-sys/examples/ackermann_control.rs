...
fn main() {
    ...
    unsafe {
        ...
        if vehicle_bp.is_null() {
            ...
            let mut count = 0;
            let vehicle_bp =
                carla_blueprint_library_filter(blueprints, default_bp_id.as_ptr(), &mut count);
            if vehicle_bp.is_null() {
        ...
        println!(
            "🚗 Spawning vehicle at ({:.1}, {:.1}, {:.1})",
            spawn_point.location.x, spawn_point.location.y, spawn_point.location.z
        );

        // Spawn vehicle
        let spawn_result =
            carla_world_spawn_actor(world, vehicle_bp, &spawn_point, ptr::null_mut());
        ...
        println!(
            ...
            controller_settings.speed_kp,
            controller_settings.speed_ki,
            controller_settings.speed_kd
        );
        println!(
            "   Accel PID: P={:.2}, I={:.2}, D={:.2}",
            controller_settings.accel_kp,
            controller_settings.accel_ki,
            controller_settings.accel_kd
        );
        ...
        apply_ackermann_control_with_feedback(
            vehicle,
            &straight_accel_control,
...
unsafe fn apply_ackermann_control_with_feedback(
    ...
    println!(
        "     Steer: {:.1}° @ {:.1}°/s",
        control.steer, control.steer_speed
    );
