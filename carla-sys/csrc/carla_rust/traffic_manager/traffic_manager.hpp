...
class FfiAction {
public:
    FfiAction(Action&& base) : inner_(base) {}

    RoadOption road_option() const { return inner_.first; }
...
public:
    FfiActionBuffer(ActionBuffer&& base) : inner_(base) {}
...
public:
    FfiTrafficManager(TrafficManager&& base) : inner_(std::move(base)) {}
