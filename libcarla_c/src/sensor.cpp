...
namespace {
    ...
    // Helper to get Sensor from Actor (using carla_actor_t since sensor is a typedef of carla_actor_t)
    ...
    std::shared_ptr<const carla::client::Sensor> GetSensor(const carla_actor_t* actor) {
        if (!actor || !actor->actor) {
            return nullptr;
        }
    ...
    carla_sensor_data_type_t IdentifySensorDataType(const carla::sensor::SensorData& data) {
...
carla_error_t carla_sensor_listen(carla_sensor_t* sensor, carla_sensor_callback_t callback, void* user_data) {
    if (!sensor || !callback) {
    ...
    try {
        ...
        auto cpp_callback = [callback, user_data](carla::SharedPtr<carla::sensor::SensorData> data) {
...
carla_sensor_data_type_t carla_sensor_data_get_type(const carla_sensor_data_t* data) {
    if (!data) {
        return CARLA_SENSOR_DATA_UNKNOWN;
    }
    return data->type;
...
carla_sensor_data_info_t carla_sensor_data_get_info(const carla_sensor_data_t* data) {
    ...
    try {
        ...
        info.sensor_transform = ConvertTransform(data->cpp_data->GetSensorTransform());
    } catch (const std::exception&) {
...
carla_transform_t carla_sensor_data_get_transform(const carla_sensor_data_t* data) {
...
carla_image_data_t carla_sensor_data_get_image(const carla_sensor_data_t* data) {
    ...
    if (!data->image_cached) {
        try {
            auto image = std::static_pointer_cast<const carla::sensor::data::Image>(data->cpp_data);
            data->image_data.width = image->GetWidth();
            data->image_data.height = image->GetHeight();
            data->image_data.fov = image->GetFOVAngle();
            data->image_data.raw_data = reinterpret_cast<const uint8_t*>(image->data());
            data->image_data.raw_data_size = image->size() * sizeof(carla::sensor::data::Color);
...
carla_error_t carla_image_data_copy_to_buffer(const carla_sensor_data_t* data, uint8_t* buffer, size_t buffer_size) {
...
carla_lidar_data_t carla_sensor_data_get_lidar(const carla_sensor_data_t* data) {
    ...
    if (!data->lidar_cached) {
        try {
            auto lidar = std::static_pointer_cast<const carla::sensor::data::LidarMeasurement>(data->cpp_data);
            data->lidar_data.points = reinterpret_cast<const carla_lidar_detection_t*>(lidar->data());
...
const carla_lidar_detection_t* carla_lidar_data_get_points(const carla_sensor_data_t* data) {
    auto lidar_data = carla_sensor_data_get_lidar(data);
...
carla_error_t carla_lidar_data_copy_points_to_buffer(const carla_sensor_data_t* data, 
    ...
    size_t required_size = lidar_data.point_count * sizeof(carla_lidar_detection_t);
...
bool carla_actor_is_sensor(const carla_actor_t* actor) {
    ...
    try {
        auto sensor = std::dynamic_pointer_cast<const carla::client::Sensor>(actor->actor);
...
carla_imu_data_t carla_sensor_data_get_imu(const carla_sensor_data_t* data) {
    ...
    if (!data->imu_cached) {
        try {
            auto imu = std::static_pointer_cast<const carla::sensor::data::IMUMeasurement>(data->cpp_data);
...
carla_vector3d_t carla_imu_data_get_accelerometer(const carla_sensor_data_t* data) {
...
carla_gnss_data_t carla_sensor_data_get_gnss(const carla_sensor_data_t* data) {
    ...
    if (!data->gnss_cached) {
        try {
            auto gnss = std::static_pointer_cast<const carla::sensor::data::GnssMeasurement>(data->cpp_data);
...
carla_collision_data_t carla_sensor_data_get_collision(const carla_sensor_data_t* data) {
    ...
    if (!data->collision_cached) {
        try {
            auto collision = std::static_pointer_cast<const carla::sensor::data::CollisionEvent>(data->cpp_data);
            ...
            data->collision_data.normal_impulse = ConvertVector3D(collision->GetNormalImpulse());
...
carla_actor_t* carla_collision_data_get_other_actor(const carla_sensor_data_t* data) {
    auto collision_data = carla_sensor_data_get_collision(data);
    return collision_data.other_actor;
}

carla_vector3d_t carla_collision_data_get_normal_impulse(const carla_sensor_data_t* data) {
    auto collision_data = carla_sensor_data_get_collision(data);
    return collision_data.normal_impulse;
}

// Stub implementations for remaining sensor types (to be completed)
carla_semantic_lidar_data_t carla_sensor_data_get_semantic_lidar(const carla_sensor_data_t* data) {
    carla_semantic_lidar_data_t semantic_lidar_data = {0};
    // TODO: Implement semantic LiDAR data access
    return semantic_lidar_data;
}

size_t carla_semantic_lidar_data_get_point_count(const carla_sensor_data_t* data) {
    return 0; // TODO: Implement
}

const carla_semantic_lidar_detection_t* carla_semantic_lidar_data_get_points(const carla_sensor_data_t* data) {
...
const carla_radar_detection_t* carla_radar_data_get_detections(const carla_sensor_data_t* data) {
    return nullptr; // TODO: Implement
}

carla_lane_invasion_data_t carla_sensor_data_get_lane_invasion(const carla_sensor_data_t* data) {
    carla_lane_invasion_data_t lane_invasion_data = {0};
    // TODO: Implement lane invasion data access
    return lane_invasion_data;
}

carla_actor_t* carla_lane_invasion_data_get_actor(const carla_sensor_data_t* data) {
    return nullptr; // TODO: Implement
}

size_t carla_lane_invasion_data_get_crossed_lane_markings_count(const carla_sensor_data_t* data) {
    return 0; // TODO: Implement
}

carla_obstacle_detection_data_t carla_sensor_data_get_obstacle_detection(const carla_sensor_data_t* data) {
...
carla_actor_t* carla_obstacle_detection_data_get_actor(const carla_sensor_data_t* data) {
    return nullptr; // TODO: Implement
}

carla_actor_t* carla_obstacle_detection_data_get_other_actor(const carla_sensor_data_t* data) {
    return nullptr; // TODO: Implement
}

float carla_obstacle_detection_data_get_distance(const carla_sensor_data_t* data) {
    return 0.0f; // TODO: Implement
}

carla_dvs_event_array_data_t carla_sensor_data_get_dvs_event_array(const carla_sensor_data_t* data) {
    carla_dvs_event_array_data_t dvs_event_array_data = {0};
    // TODO: Implement DVS event array data access
    return dvs_event_array_data;
}

size_t carla_dvs_event_array_data_get_event_count(const carla_sensor_data_t* data) {
    return 0; // TODO: Implement
}

const carla_dvs_event_t* carla_dvs_event_array_data_get_events(const carla_sensor_data_t* data) {
    return nullptr; // TODO: Implement
}
