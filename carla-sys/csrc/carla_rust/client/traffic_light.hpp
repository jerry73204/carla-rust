...
public:
    FfiTrafficLight(SharedPtr<TrafficLight>&& base) : inner_(std::move(base)) {}
    ...
    TrafficLightState GetState() const { return inner_->GetState(); }

    void SetGreenTime(float green_time) const { inner_->SetGreenTime(green_time); }

    float GetGreenTime() const { return inner_->GetGreenTime(); }

    void SetYellowTime(float yellow_time) const { inner_->SetYellowTime(yellow_time); }

    float GetYellowTime() const { return inner_->GetYellowTime(); }

    void SetRedTime(float red_time) const { inner_->SetRedTime(red_time); }

    float GetRedTime() const { return inner_->GetRedTime(); }

    float GetElapsedTime() const { return inner_->GetElapsedTime(); }

    void Freeze(bool freeze) const { inner_->Freeze(freeze); }

    bool IsFrozen() const { return inner_->IsFrozen(); }

    uint32_t GetPoleIndex() const { return inner_->GetPoleIndex(); }
    ...
    void ResetGroup() const { inner_->ResetGroup(); }
