...
public:
    ...
    float GetHorizontalAngle() const { return inner_->GetHorizontalAngle(); }

    uint32_t GetChannelCount() const { return inner_->GetHorizontalAngle(); }

    uint32_t GetPointCount(size_t channel) const { return inner_->GetPointCount(channel); }
    ...
    const FfiSemanticLidarDetection& at(size_t pos) const {
        ...
        const FfiSemanticLidarDetection& new_ =
            reinterpret_cast<const FfiSemanticLidarDetection&>(orig);
        return new_;
    }
