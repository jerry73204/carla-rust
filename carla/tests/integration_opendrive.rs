...
fn test_opendrive_parsing() {
    ...
    if let Err(e) = result {
        eprintln!("Test requires CARLA server running: {}", e);
        panic!("Failed to connect to CARLA server");
    }
}
    ...
    let result: CarlaResult<()> = (|| {
        ...
        if let Some(spawn_transform) = spawn_points.iter().next() {
            if let Some(waypoint) = map.waypoint(spawn_transform.location) {
                ...
                for _ in 0..5 {
                    let next_waypoints = current.next(10.0)?;
    ...
    let result: CarlaResult<()> = (|| {
        ...
        if let Some(first_junction_wp) = junction_waypoints.first() {
            if let Some(junction) = map.junction(first_junction_wp) {
                ...
                let junction_wps = junction.waypoints(LaneType::Driving)?;
        ...
        if let Some(first_wp) = waypoints.get(0) {
            ...
            for i in 0..5 {
                ...
                if let Some(current) = map.waypoint(current_location) {
                    if let Some(next) = current.next(10.0)?.get(0) {
                        if next.road_id == current.road_id {
...
fn test_parse_geometry() {
    ...
    let result: CarlaResult<()> = (|| {
        ...
        let total_geometry =
            line_count + arc_count + spiral_count + poly3_count + param_poly3_count;
        ...
        for i in 0..test_count {
            if let Some(waypoint) = waypoints.get(i) {
                ...
                if i < 3 {
                    ...
                    println!("    Has left lane: {}", waypoint.left_lane()?.is_some());
                    println!("    Has right lane: {}", waypoint.right_lane()?.is_some());
                }
            }
        }
...
fn test_get_waypoint() {
    ...
    use carla::{client::Client, error::CarlaResult, geom::Location};
    ...
    let result: CarlaResult<()> = (|| {
        ...
        if let Some(waypoint) = map.waypoint(origin) {
            ...
            if let Some(left_waypoint) = waypoint.left_lane()? {
                println!("  Has left lane: lane_id={}", left_waypoint.lane_id);
            }

            if let Some(right_waypoint) = waypoint.right_lane()? {
