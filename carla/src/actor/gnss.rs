...
impl GNSS {
    /// Create a GNSS sensor from a carla-cxx SensorWrapper.
    ...
    /// Create a GNSS sensor from an actor by casting.
    ...
    pub fn listen_async(&self) -> Result<tokio::sync::mpsc::Receiver<GNSSData>, SensorError> {
        ...
        todo!("GNSS::listen_async not yet implemented with carla-cxx FFI")
    }
...
impl ActorT for GNSS {
    fn id(&self) -> ActorId {
        // GNSS doesn't have a direct GetId method, need to cast to Actor
        let actor_ptr = carla_cxx::ffi::Sensor_CastToActor(self.inner.get_inner_sensor());
        if actor_ptr.is_null() {
            panic!("Internal error: Failed to cast GNSS to Actor");
        }
    ...
    fn bounding_box(&self) -> crate::geom::BoundingBox {
        // GNSS doesn't have a direct GetBoundingBox method, need to cast to Actor
        let actor_ptr = carla_cxx::ffi::Sensor_CastToActor(self.inner.get_inner_sensor());
        if actor_ptr.is_null() {
            panic!("Internal error: Failed to cast GNSS to Actor");
        }
...
impl SensorT for GNSS {
    ...
    fn get_attribute(&self, name: &str) -> Option<String> {
        ...
        todo!("GNSS::get_attribute not yet implemented - missing FFI function Sensor_GetAttribute")
    }
